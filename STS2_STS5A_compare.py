...
fftRef = np.fft.fft(trRef.data,n=pad)
...
fftNom = np.fft.fft(trNom.data,n=pad)
...
plt.ylabel('Magnitude (displacement)')
plt.subplot(212)
plt.grid(True, which='both')
...
PSDfreqs, PSDRef = signal.welch(trRef.data, return_onesided=True, fs=samprate, nperseg=nfft, noverlap=overlap, scaling='density')
PSDfreqs, PSDNom = signal.welch(trNom.data, return_onesided=True, fs=samprate, nperseg=nfft, noverlap=overlap, scaling='density')
